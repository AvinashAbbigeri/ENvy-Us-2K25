export const Cards = [
    {
        url: "https://unstop.com/events/tech-debate-apoorv-2024-iiit-kottayam-XXXXXXX",
        image: "/assets/eventsImages/TechDebate.png",
        color: '#F1C40F',
        event_name: "Tech Debate",
        description: "Put your debating skills to the test! In Tech Debate, you'll be given a topic to debate on the spot. Whether you're for or against, the floor is yours to persuade and present your arguments. Sharpen your critical thinking and communication abilities in this exhilarating event!",
        category: "Technical"
    },
    {
        url: "https://unstop.com/events/tech-quest-apoorv-2024-iiit-kottayam-XXXXXXX",
        image: "/assets/eventsImages/TechQuest.png",
        color: '#2E4053',
        event_name: "Tech Quest",
        description: "Ready for an interview experience like no other? Tech Quest offers a platform for participants to engage in a professional interview simulation. Perfect for aspiring tech professionals, showcase your knowledge, problem-solving skills, and creativity as you navigate through mock interviews with experts!",
        category: "Technical"
    },
    {
        url: "https://unstop.com/events/logo-rithm-apoorv-2024-iiit-kottayam-XXXXXXX",
        image: "/assets/eventsImages/LogoRithm.png",
        color: '#F39C12',
        event_name: "Logo-rithm",
        description: "Do you have an eye for design? Join Logo-rithm, where you will be tasked with redesigning an existing logo to make it more modern, appealing, and recognizable. Put your creativity to the test as you reimagine iconic designs!",
        category: "Technical"
    },
    {
        url: "https://unstop.com/events/beyond-the-logo-apoorv-2024-iiit-kottayam-XXXXXXX",
        image: "/assets/eventsImages/BeyondTheLogo.png",
        color: '#9B59B6',
        event_name: "Beyond the Logo",
        description: "Can you guess a logo from multiple hints? Beyond the Logo is a fun and challenging event where participants must identify logos based on partial images or clues. Test your knowledge of branding and your ability to recognize logos in this exciting competition!",
        category: "Technical"
    },
    {
        url: "https://unstop.com/events/quiz-code-apoorv-2024-iiit-kottayam-XXXXXXX",
        image: "/assets/eventsImages/QuizCode.png",
        color: '#3498DB',
        event_name: "Quiz Code",
        description: "Gear up for an intellectual challenge! Quiz Code is a tech-themed quiz competition combined with coding challenges. Test your knowledge in both technology and programming as you solve tricky problems and answer thought-provoking questions!",
        category: "Technical"
    },
    {
        url: "https://unstop.com/events/mind-maze-apoorv-2024-iiit-kottayam-XXXXXXX",
        image: "/assets/eventsImages/MindMaze.png",
        color: '#E74C3C',
        event_name: "Mind Maze",
        description: "Ready to crack some mind-bending riddles and puzzles? Mind Maze brings you a series of riddles, case studies, and mysteries that require sharp thinking and out-of-the-box problem-solving skills. Get ready to challenge your brain and solve the unsolvable!",
        category: "Technical"
    }
];


export const TechnicalTeam = [
    // {
    //     name: "Mohit Gupta",
    //     image: "/assets/techTeamImages/mohit.jpg",
    //     linkedin: "https://www.linkedin.com/in/mohit-gupta0111/",
    //     instagram: "https://www.instagram.com/_mohit_gupta01/?utm_source=qr&r=nametag",
    // },
    // {
    //     name: "Gaurav Yadav",
    //     image: "/assets/techTeamImages/gaurav.jpg",
    //     linkedin: "https://www.linkedin.com/in/gaurav-yadav-091734287/",
    //     instagram: "https://www.instagram.com/gaurav2201yadav?igsh=MWlybjA4NmFxendxYw==",
    // },
    // {
    //     name: "Atul Singh",
    //     image: "/assets/apoorvTeamImages/atul.jpg",
    //     linkedin: "https://www.linkedin.com/in/atul-singh-1739b5241/",
    //     instagram: "https://www.instagram.com/i.m.atul__1706?igsh=aXZvYTRnbHZvZzN4",
    // },
    // {
    //     name: "Mahendra",
    //     image: "/assets/techTeamImages/mahendra.jpg",
    //     linkedin: "https://www.linkedin.com/in/mahendra-kumar-dambiwal-59351b23b/",
    //     instagram: "https://www.instagram.com/mahi.hanu121/",
    // },
    // {
    //     name: "Abhishek Raj",
    //     image: "/assets/techTeamImages/abhishek.jpg",
    //     linkedin: "https://www.linkedin.com/in/abhishek-raj6299/",
    //     instagram: "https://www.instagram.com/crimmson.guy?igsh=YTlnNGZzNjJ5bmd0",
    // },
]

export const ApoorvTeam = [
    // {
    //     name: "Aditya",
    //     image: "/assets/apoorvTeamImages/aditya.jpg",
    // },
    // {
    //     name: "Atul",
    //     image: "/assets/apoorvTeamImages/atul.jpg",
    // },
    // {
    //     name: "Saketh",
    //     image: "/assets/apoorvTeamImages/saketh.jpg",
    // },
    // {
    //     name: "Blaize Tom",
    //     image: "/assets/apoorvTeamImages/blaize.jpg",
    // },
    // {
    //     name: "Ajay",
    //     image: "/assets/apoorvTeamImages/ajay.jpg",
    // },
    // {
    //     name: "Shivani",
    //     image: "/assets/apoorvTeamImages/shivani.jpg",
    // },
    // {
    //     name: "Priyatham",
    //     image: "/assets/apoorvTeamImages/priyatam.jpg",
    // },
    // {
    //     name: "Yaseen",
    //     image: "/assets/apoorvTeamImages/yaseen.jpg",
    // },
    // {
    //     name: "Rakshit",
    //     image: "/assets/apoorvTeamImages/rakshit.jpg",
    // },
]

const timelineArray = [
    { time: "Sabhangana (Ground Floor)", event: "DEBATE" },
    { time: "Room 324 (3rd Floor)", event: "SUDOKU" },
    { time: "Room 304 (3rd Floor)", event: "GAMING (BGMI)" },
    { time: "CS LAB-1 (3rd Floor)", event: "CODING" },
    { time: "(3rd Floor)", event: "TREASURE HUNT" },
    { time: "Room 322 (3rd Floor)", event: "LOGO DESIGN" },
    { time: "(3rd Floor)", event: "VIDEOGRAPHY" },
    { time: "Room 325 (3rd Floor)", event: "Electronic PPT" },
    { time: "Room 307 (3rd Floor)", event: "RUBIK'S CUBE" },
    { time: "Sabhangana (Ground Floor)", event: "IT QUIZ" },
    { time: "Room 321 (3rd Floor)", event: "MATHS PPT" },
    { time: "1:00 PM", event: "LUNCH" },
    { time: "Electronics Lab (4th Floor)", event: "Electronic Circuit Analysis" },
    { time: "AUDITORIUM", event: "Fashion Walk" },
  ];
  
  export default timelineArray;
  